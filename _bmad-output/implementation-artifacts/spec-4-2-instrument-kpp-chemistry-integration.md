...
- Given a non-DEBUG production build, when the same run executes, then instrumentation adds no measurable overhead (NFR-4). **Met for CBMZ structurally** -- the `#ifdef DEBUG` guard is confirmed absent from production build output, on all three vendors, on the current tree. **Met for CB6r2 by code-inspection only** -- guard absence is guaranteed by dead code (`GAS_CB6r2` unwired from the build), never observed in actual build output on any vendor.
...
**Runtime verification: not achieved, accepted as code-inspection-only.** Three real Slurm attempts (jobs 51802833, 51802864, 51802912) against a chemistry-enabled EUR12 CORDEX fixture each crashed in a different pre-existing chemistry/boundary support routine before ever reaching `cbmz_integrate` (confirmed 0 occurrences across all per-rank debug files from the last attempt). A retry against the same crash with a different toolchain (jobs 51803006/51803048) ruled out a library-version cause. An attempt to route around it via a different dynamical-core configuration hit the `iproj='ROTLLR'`/`idynamic=3` restriction (jobs 51812033/51812133). Standing up a fresh, smaller domain instead (job 51813820, terrain/ICBC, passed cleanly) let the DEBUG binary actually run, which then surfaced, in order: a namelist-window fix, the truncated-IFS-file workaround (see `deferred-work.md`), an `ichsursrc`/`ichebdy` default-handling fix, and finally the `aeroppt` bug, which blocked further progress. Separately, `mod_lm_interface.F90:930` (job 51814413) was the further, distinct crash this story stopped at. CB6r2 cannot be runtime-verified at all — unreachable from any build. See Spec Change Log and `deferred-work.md` for the six bugs/constraints this surfaced.
