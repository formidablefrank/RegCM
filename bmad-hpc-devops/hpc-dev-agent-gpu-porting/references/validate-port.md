...
Numerical correctness is only half of "validated." Per `openacc-best-practices.md`'s iterative Porting Cycle, a port that's numerically correct but was never re-profiled can still be a net loss — hand it back to `hpc-dev-agent-profiler` to confirm the data-locality and parallelism-mapping choices from `port-routine.md` actually delivered a speedup, rather than declaring the routine done at correctness alone.
