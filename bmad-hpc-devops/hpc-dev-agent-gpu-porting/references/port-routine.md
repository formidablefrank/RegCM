...
When the routine escalates to OpenACC, apply `openacc-best-practices.md`: choose `parallel loop` over `kernels` once independence is established (the normal case here, since the aliasing check already ran in `evaluate-candidacy.md`); structure data movement as a region spanning the routine's actual data lifetime rather than a copy per call, using `copyin`/`copyout`/`create` deliberately instead of a blanket `copy`; map RegCM5's `(iy, jx)` horizontal decomposition and vertical `k` loop to gang/worker/vector deliberately, reaching for `collapse` across the horizontal indices when the vertical loop is short or carries a dependency. Confirm every one of these choices against `-Minfo=accel` output — a clean compile is not confirmation that a region parallelized or that data stayed resident.
